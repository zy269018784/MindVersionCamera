...
/// \param [in] pContext 回调函数的附加参数，在回调函数被调用时该附加参数会被传入，可以为NULL。多用于多个相机时携带附加信息。
/// \param [out] pCallbackOld 用于返回之前设置的回调函数。可以为NULL。
/// \param [in] uFlags 标志位，bit0: 1. 手动释放帧缓冲区 0. 回调完成后自动释放帧缓冲区  bit1-31: 必须为0
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
/// \brief Sets the image capture's callback function. When a new frame of image data is captured, the callback function pointed to by pCallBack is called.
/// \param [in] hCamera Camera handle.
...
/// \param [in] pContext Additional parameters of the callback function, which will be passed in when the callback function is called, can be NULL. Use additional information when used with multiple cameras.
/// \param [out] pCallbackOld Returns the previously set callback function. Can be NULL.
///\ param [in] uFlags flag bit, bit0: 1. Manually release frame buffer. 0. Automatically release frame buffer after callback completion  Bit1-31: must be 0
/// \return Returns CAMERA_STATUS_SUCCESS(0) successfully. Otherwise, it returns a non-zero error code. Please refer to the definition of the error code in CameraStatus.h.
MVSDK_API CameraSdkStatus __stdcall CameraSetCallbackFunctionEx(
	CameraHandle        hCamera,
	CAMERA_SNAP_PROC    pCallBack,
	PVOID               pContext,
	CAMERA_SNAP_PROC*   pCallbackOld,
...
/// \brief 设置图像捕获的异常处理函数。
/// \param [in] hCamera 相机的句柄。
/// \param [in] pExceptionHandler 处理函数指针。
/// \param [in] pContext 处理函数的附加参数，在处理函数被调用时该附加参数会被传入，可以为NULL。多用于多个相机时携带附加信息。
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
/// \brief Set the exception handling function for image capture.
/// \param [in] hCamera Camera handle.
/// \param [in] pExceptionHandler Handler function pointer.
/// \param [in] pContext Additional parameters of the handler function, which will be passed in when the handler function is called, can be NULL. Use additional information when used with multiple cameras.
/// \return Returns CAMERA_STATUS_SUCCESS(0) successfully. Otherwise, it returns a non-zero error code. Please refer to the definition of the error code in CameraStatus.h.
MVSDK_API CameraSdkStatus __stdcall CameraSetGrabExceptionHandler(
	CameraHandle        hCamera,
	SNAP_EXCEPTION_HANDLER pExceptionHandler,
	PVOID               pContext
	);
...
/// \return Returns CAMERA_STATUS_SUCCESS(0) successfully. Otherwise, it returns a non-zero error code. Please refer to the definition of the error code in CameraStatus.h.
MVSDK_API CameraSdkStatus __stdcall CameraGetDevTimeStamp(
	CameraHandle    hCamera,
	UINT*			pTimeStampL32,
	UINT*			pTimeStampH32
	);
