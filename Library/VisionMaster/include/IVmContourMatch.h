 ...
 * Note：@~chinese 轮廓匹配模块接口声明 @~english Interface for the ContourMatch tool.
 *
 ***************************************************************************************************/
...
namespace VisionMasterSDK
{
    namespace IMVSContourMatchModu
    {
        ...
        /// @~chinese 多模板排序方式 @~english RunParam_NMSSort
        typedef enum _NMSSortTypeEnum
        {
            /// @~chinese 按分数降序排序 @~english Score: High to Low
            NMSSortType_Score = 0x2,

            /// @~chinese 按角度降序排序 @~english Angle: Big to Small
            NMSSortType_Angle = 0x3,
    ...
    /// @{
        ...
        public:
            ...
            /// @~chinese 多模板重叠率，范围：[0,100](废弃)   @~english NMS Overlap Threshold，range:[0,100](Obsoleted)
            __declspec(property(put = SetNMSMaxOverlap, get = GetNMSMaxOverlap)) int NMSMaxOverlap;

            virtual int GetNMSMaxOverlap() = 0;

            virtual void SetNMSMaxOverlap(int value) = 0;

            /// @~chinese 多模板排序方式(废弃)   @~english RunParam_NMSSort(Obsoleted)
            __declspec(property(put = SetNMSSortType, get = GetNMSSortType)) NMSSortTypeEnum NMSSortType;

            virtual NMSSortTypeEnum GetNMSSortType() = 0;

            virtual void SetNMSSortType(NMSSortTypeEnum value) = 0;

            /// @~chinese 多模板重叠率，范围：[0,100]   @~english NMS Overlap Threshold，range:[0,100]
            __declspec(property(put = SetMultiModelMaxOverlap, get = GetMultiModelMaxOverlap)) int MultiModelMaxOverlap;

            virtual int GetMultiModelMaxOverlap() = 0;

            virtual void SetMultiModelMaxOverlap(int value) = 0;
            ...
            /// @~chinese 横切角度范围，范围：[-45,45]   @~english SkewX Range，range:[-45,45]
            __declspec(property(put = SetSkewXStart, get = GetSkewXStart)) int SkewXStart;

            virtual int GetSkewXStart() = 0;

            virtual void SetSkewXStart(int value) = 0;

            /// @~chinese 横切角度范围，范围：[-45,45]   @~english SkewX Range，range:[-45,45]
            __declspec(property(put = SetSkewXEnd, get = GetSkewXEnd)) int SkewXEnd;

            virtual int GetSkewXEnd() = 0;

            virtual void SetSkewXEnd(int value) = 0;
            ...
            /// @~chinese 是否获取轮廓   @~english Contour Enabled
            ...
            /// @~chinese 是否输出轮廓   @~english ContourPoint Enabled
            __declspec(property(put = SetOutLinePointEnable, get = GetOutLinePointEnable)) bool OutLinePointEnable;

            virtual bool GetOutLinePointEnable() = 0;

            virtual void SetOutLinePointEnable(bool value) = 0;
            ...
            /// @~chinese X尺度判断   @~english X Scale Check
            ...
            /// @~chinese X尺度范围，范围：[0.0,10]   @~english X Scale Range，range:[0.0,10]
            ...
            /// @~chinese Y尺度判断   @~english Y Scale Check
            ...
            /// @~chinese Y尺度范围，范围：[0,10]   @~english Y Scale Range，range:[0,10]
        ...
        public:
			 ...
			 */
             ...
             /**
             * @brief 轮廓点集
             * @param nIndex [IN] 轮廓点索引，从0开始计数
             */
             virtual PointF GetContourPoints(int nIndex) = 0;

             /**
             * @brief 轮廓点所属轮廓序号
             * @param nIndex [IN] 轮廓点索引，从0开始计数
             */
             virtual int GetContourIndex(int nIndex) = 0;
